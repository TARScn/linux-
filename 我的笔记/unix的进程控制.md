...
父进程中返回父进程的pid，子进程中返回0；常用于区分当前是父进程还是子进程。
...
__*execlp() vs execvp()*__：

execlp() 使用可变参数列表来传递参数，需要手动列出所有参数。

execvp() 使用一个参数数组（char *argv[]），更适合动态生成参数列表。

__*execv*__、__*execl*__：不能使用PATH环境变量，需要给出程序完整路径。
